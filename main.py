...
# ══════════════════════════════════════════════════════════════════════════════
# MODE: TRAIN
# Feature matrix is saved to disk as a numpy memmap so:
#   (a) a crash during training never forces full re-extraction
#   (b) the matrix is memory-mapped — OS loads pages on demand, not all at once
# ══════════════════════════════════════════════════════════════════════════════
def run_train(cfg: dict, max_rows: int = None):
    """
    Train AE1 -> IF -> AE2 ensemble on all ingested DuckDB logs.

    Crash-safe: feature matrix is cached to disk after extraction.
    Memory-safe: matrix is memory-mapped; scaler is fitted incrementally.
    GPU-aware:  automatically uses CUDA / MPS / CPU.
    """
    from ingestion_engine import LogIngestionEngine
    ...
    # ── Paths for crash-safe disk cache ──────────────────────────────────────
    Path(cfg["model_dir"]).mkdir(parents=True, exist_ok=True)
    matrix_path  = Path(cfg["model_dir"]) / "feature_matrix.npy"
    meta_path    = Path(cfg["model_dir"]) / "feature_meta.pkl"

    # ── Phase 1: Feature extraction (skip if cached) ──────────────────────────
    if matrix_path.exists() and meta_path.exists():
        logger.info(
            "Phase 1/4: Loading cached feature matrix from disk "
            "(extraction already complete)"
        )
        feature_matrix = np.load(str(matrix_path), mmap_mode="r")
        with open(meta_path, "rb") as f:
            all_metadata = pickle.load(f)
        total_rows = len(all_metadata)
        logger.info(f"  Loaded: {feature_matrix.shape}  from {matrix_path}")
    else:
        ...
        # Save to disk — enables crash-safe resume AND memory-mapped access
        logger.info(f"  Saving feature matrix to {matrix_path} ...")
        np.save(str(matrix_path), feature_matrix)
        with open(meta_path, "wb") as f:
            pickle.dump(all_metadata, f)
        logger.info("  Feature matrix cached. Training will resume from here if interrupted.")

        # Reload as memmap so downstream training never needs the full array in RAM
        del all_features
        feature_matrix = np.load(str(matrix_path), mmap_mode="r")
